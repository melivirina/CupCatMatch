...
public class BoardManager : MonoBehaviour
{
        ...
        for (int x = 0; x < xSize; x++)
        {
            for (int y = 0; y < ySize; y++)
            {
                if (x != 2 || y != 3)
                {
                    ...
                    Sprite newSprite = possibleCharacters[UnityEngine.Random.Range(0, possibleCharacters.Count - 1)];
    ...
    public IEnumerator FindNullTiles()
    {
        ...
        for (int x = 0; x < xSize; x++)
        {
            for (int y = 0; y < ySize; y++)
            {
                if ((x == 2 && y == 3) || tiles[x, y].GetComponent<Bomb>() != null)
                {
        ...
        for (int x = 0; x < xSize; x++)
        {
            for (int y = 0; y < ySize; y++)
            {
                ...
                if ((x != 2 || y != 3) && tiles[x, y].GetComponent<Tile>() != null)
    ...
    private IEnumerator ShiftTilesDown(int x, int yStart, float shiftDelay = .06f)
    {
        ...
        for (int y = yStart; y < ySize; y++)
        {
            ...
            if ((x != 2 || y != 3) && tiles[x, y].GetComponent<Tile>() != null)
            {
                ...
                {
                    if (tiles[x, y + 1].GetComponent<Bomb>() != null && y == ySize - 2)
                    {
                        render.sprite = GetNewSprite(x, ySize - 1);
                    }
                    else if (tiles[x, y + 1].GetComponent<Bomb>() != null && y < ySize - 2)
                    {
    ...
    public IEnumerator FindBomb(float startX, float startY)
    {
        ...
        for (int x = 0; x < xSize; x++)
        {
            for (int y = 0; y < ySize; y++)
            {
                ...
                if (tiles[x, y].GetComponent<Bomb>() != null
    ...
    {
        List<Sprite> possibleCharacters = new List<Sprite>();
        //possibleCharacters = SpriteList.cats;
        possibleCharacters.AddRange(pussy_use);
        ...
        return possibleCharacters[UnityEngine.Random.Range(0, possibleCharacters.Count - 1)];
    }
